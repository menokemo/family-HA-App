...
type Props = { label: string; icon: string; color: string; onConfirm: () => void };

const KNOB = 56;

export function SwipeToConfirm({ label, icon, color, onConfirm }: Props) {
  ...
  return (
    <View style={s.track} onLayout={onLayout}>
      ...
      <Animated.View style={[s.knob, { backgroundColor: color, transform: [{ translateX: pan }] }]} {...responder.panHandlers}>
...
const s = StyleSheet.create({
  ...
  knob: { position: 'absolute', left: 4, width: KNOB, height: KNOB, borderRadius: KNOB / 2, alignItems: 'center', justifyContent: 'center' },
});
